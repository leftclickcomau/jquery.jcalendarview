...
;(function($) {
  ...
  */
  $.jcalendarview.defaultOptions = {
    ...
    monthSpanClass: 'cvMonth',
    yearSpanClass: 'cvYear',
  ...
  $.extend($.jcalendarview.prototype, {
    ...
    */
    createMonthYearDiv : function(date) {
      ...
      return div.append(previousMonthLink)
                .append(nextMonthLink)
                .append($('<span class="' + this.options.monthSpanClass + '">' + date.toString('MMMM') + '</span>'))
                .append($('<span> </span>'))
                .append($('<span class="' + this.options.yearSpanClass + '">' + date.toString('yyyy') + '</span>'));
    },
    ...
    */
    createDayDiv : function(date, isOutOfMonth, dateDiff) {
      ...
      div.parent().attr('id', this.options.dayClass + '_' + date.toString('yyyyMMdd'));
    ...
    */
    move : function(delta) {
      var newDate = this.date.addMonths(delta);
      var firstOfMonth = newDate.clone().moveToFirstDayOfMonth();
      var endOfMonth = newDate.clone().moveToLastDayOfMonth();
      if (firstOfMonth.compareTo(this.options.maxDate) <= 0 && endOfMonth.compareTo(this.options.minDate) >= 0) {
        this.date = newDate;
        this.container.html('');
        this.render();
      }
    ...
    */
    render : function() {
      ...
      date.moveToDayOfWeek(0, -1);
      if (this.options.alwaysSixWeeks) {
        for (var i=0; i<42; i++) {
      ...
      // Show/hide the next/previous month buttons
      var firstOfNextMonth = this.date.clone().moveToFirstDayOfMonth().addMonths(1);
      if (firstOfNextMonth.compareTo(this.options.maxDate) > 0) {
        $('a.'+this.options.nextMonthLinkClass).css('visibility', 'hidden');
      }
      var endOfPrevMonth = this.date.clone().moveToLastDayOfMonth().addMonths(-1);
      if (endOfPrevMonth.compareTo(this.options.minDate) < 0) {
        $('a.'+this.options.previousMonthLinkClass).css('visibility', 'hidden');
      }
